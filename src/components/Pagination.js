...
export function Pagination() {
  ...
  const pageClickHandler = (index) => {
    ...
    window.history.pushState({}, '', URLWithPage);
  ...
  useEffect(() => {
    ...
    if (pages.length > 0) {
      if (pageNumber > 0 && pageNumber != activePage) {
        ...
        setApiURL(pages[pageNumber]);
      }
      // Рабочий вариант, но при загрузке видно сначала 0-ю страницу, а потом страницу полученную из параметров
    }
  }, [pages]);
